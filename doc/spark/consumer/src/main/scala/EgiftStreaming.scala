...
object EgiftStreaming {

  def main(args: Array[String]) {

    val jagaurTopic = "jaguar-sql-messages"
    kafkaConsume("localhost:2181", "test-group", jagaurTopic, 10)
  }
  ...
  def kafkaConsume(zkQuorum: String, group: String, topics: String, threads: Int)  {
	...
	// println("*** In each record:  threadID2=" + threadId2 + " *************")
        val sqlstr = "insert into starbucks.egift_item values (" + record._2 + ")"
	// println( sqlstr )
	db.runSql(sqlstr)
      }
...
class EgiftDB extends Serializable {
 
    Class.forName("com.jaguar.jdbc.JaguarDriver");
    @transient lazy val conn = DriverManager.getConnection("jdbc:jaguar://127.0.0.1:7777/test", "admin", "jaguar" );
